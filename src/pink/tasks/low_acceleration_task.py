...
from .posture_task import AccelerationTask


class LowAccelerationTask(AccelerationTask):
    ...
    def set_last_integration(self, configuration: Configuration, v_prev: np.ndarray, dt) -> None:
        ...
        self.Delta_q_prev = v_prev[nv:] * dt
    ...
    def compute_error(self, configuration: Configuration) -> np.ndarray:
        ...
        if self.Delta_q_prev is not None:
            Delta_q_prev: np.ndarray = self.Delta_q_prev
        else:
            _, nv = get_root_joint_dim(configuration.model)
            Delta_q_prev: np.ndarray = np.zeros(configuration.model.nv)[nv:]          
        return -Delta_q_prev
