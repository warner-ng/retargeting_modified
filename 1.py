...
def blend_motion(array, skip):
    array = array[:int(int(array.shape[0] / skip) * skip)] # 目的 ：确保数组的长度能够被 skip 整除，以便后续分组操作
    array = array.reshape((int(array.shape[0] / skip), skip, -1))# reshape 将数组重塑为三维数组：第一维：分组的数量。第二维：每组的大小（即 skip）第三维：保留数组的其余维度（-1 表示自动推导）
...
if __name__ == "__main__":# 解析参数-初始化-指定任务-ik求解-存储
    ...
    if not args.headless: # 两个visualizer
...
# 这是一个大循环（一直到末尾）
    for mocap_name, mocap_data in tqdm(pickle.load(open(args.data,"rb")).items()):  # mocap_name是键；mocap——data是值；遍历1次，取1次键key和1次值value
        ...
        # SkeletonBinding Human and Humanoid 这样做之后，就可以把v_template（smpl的）转换到v_shaped（机器人的）
